...
class MessageTemplate:
    ...
    def get_help_message() -> str:
        ...
        return (
            ...
            f"🔧 <i>Для поддержки: {settings.support_contact}</i>"
        )
    ...
    @staticmethod
    def get_insufficient_balance_message(current_balance: float, required_amount: int, missing_amount: float, currency: str = "TON") -> str:
        """
        Получить специальное сообщение о недостаточном балансе.
        
        Args:
            current_balance: Текущий баланс пользователя
            required_amount: Требуемая сумма для покупки
            missing_amount: Недостающая сумма
            currency: Валюта
            
        Returns:
            str: Отформатированное сообщение о недостаточном балансе
        """
        return (
            f"{MessageTemplate.EMOJI_WARNING} <b>Недостаточно средств на балансе</b> {MessageTemplate.EMOJI_WARNING}\n\n"
            f"💰 <b>Ваш текущий баланс:</b> {current_balance:.2f} {currency}\n"
            f"⭐ <b>Стоимость {required_amount} звезд:</b> {required_amount:.2f} {currency}\n"
            f"❌ <b>Не хватает:</b> {missing_amount:.2f} {currency}\n\n"
            f"💡 <b>Что можно сделать:</b>\n"
            f"   💳 <i>Пополнить баланс на недостающую сумму</i>\n"
            f"   ⭐ <i>Выбрать меньшее количество звезд</i>\n"
            f"   📊 <i>Проверить историю операций</i>\n\n"
            f"🔄 <i>Используйте кнопки ниже для выбора действия</i>"
        )